...
export function ItemCardsWithDrawer({ items, collections, isImageGrid }: ItemCardsWithDrawerProps) {
  ...
  return (
    <>
      <div
        className={
          isImageGrid ? 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3' : 'grid grid-cols-1 md:grid-cols-2 gap-3'
        }
      >
        {items.map((item) =>
          isImageGrid ? (
